...
if (len > E.screencols - 8) len = E.screencols - 8;
abAppend(ab, status, len);

/* Fill the gap between status text and percentage indicator.
 * We write screencols-1 visible characters total (len + fill + 7),
 * then CSI K fills the last column using the reverse-video
 * background. This avoids writing exactly screencols characters,
 * which triggers auto-wrap on terminals without deferred wrap
 * and causes a double line advance in multi-window layouts. */
...
/* Erase to end of line (fills last column in reverse video),
 * then reset formatting and advance to next line. */
