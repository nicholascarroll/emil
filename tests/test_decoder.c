 ...
 * every mapped key, both wait classes, unknown and malformed
 ...
 * ESC"; exhaustion returns 0, modelling the two cases in which the
 * real source reports that no byte will come: a Meta-prefix wait
 * abandoned by a signal, and end of input.  (There is no timeout;
 * sequence bytes are waited for indefinitely.) */
