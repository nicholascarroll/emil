...
static struct category categories[CAT_COUNT] = {
	...
	[CAT_DEVANAGARI_INDIC] = { "Devanagari/Indic (U+0900..0DFF)", 0, 0, 0,
				   0 },
	...
	[CAT_HIRAGANA_KATAKANA] = { "Hiragana/Katakana (U+3040..30FF)", 0, 0, 0,
				    0 },
	[CAT_COMBINING_MARKS] = { "Combining Marks (U+0300..036F)", 0, 0, 0,
				  0 },
	[CAT_BOX_DRAWING] = { "Box Drawing (U+2500..257F)", 0, 0, 0, 0 },
	[CAT_MATH_SYMBOLS] = { "Math Symbols (U+2200..22FF)", 0, 0, 0, 0 },
	[CAT_EMOJI_BASIC] = { "Emoji: Misc Symbols (U+2600..26FF)", 0, 0, 0,
			      0 },
	[CAT_EMOJI_SUPPLEMENTAL] = { "Emoji: Supplemental (U+1F300..1F5FF)", 0,
				     0, 0, 0 },
	[CAT_EMOJI_EXTENDED] = { "Emoji: Faces+ (U+1F600..1FAFF)", 0, 0, 0, 0 },
	[CAT_PRIVATE_USE] = { "Private Use (U+E000..F8FF)", 0, 0, 0, 0 },
	[CAT_MISC_SYMBOLS] = { "Misc Symbols (U+2000..2BFF excl above)", 0, 0,
			       0, 0 },
...
static int classify(uint32_t cp) {
	...
	if (cp >= 0x0300 && cp <= 0x036F)
		return CAT_COMBINING_MARKS;
	if (cp >= 0x0370 && cp <= 0x052F)
		return CAT_GREEK_CYRILLIC;
	if (cp >= 0x0590 && cp <= 0x06FF)
		return CAT_ARABIC_HEBREW;
	if (cp >= 0x0900 && cp <= 0x0DFF)
		return CAT_DEVANAGARI_INDIC;
	if (cp >= 0x2200 && cp <= 0x22FF)
		return CAT_MATH_SYMBOLS;
	if (cp >= 0x2500 && cp <= 0x257F)
		return CAT_BOX_DRAWING;
	if (cp >= 0x2600 && cp <= 0x26FF)
		return CAT_EMOJI_BASIC;
	if (cp >= 0x2000 && cp <= 0x2BFF)
		return CAT_MISC_SYMBOLS;
	if (cp >= 0x3040 && cp <= 0x30FF)
		return CAT_HIRAGANA_KATAKANA;
	if (cp >= 0x4E00 && cp <= 0x9FFF)
		return CAT_CJK_UNIFIED;
	if (cp >= 0xAC00 && cp <= 0xD7AF)
		return CAT_HANGUL;
	if (cp >= 0xE000 && cp <= 0xF8FF)
		return CAT_PRIVATE_USE;
	if (cp >= 0xF900 && cp <= 0xFAFF)
		return CAT_CJK_COMPAT;
	if (cp >= 0x1F300 && cp <= 0x1F5FF)
		return CAT_EMOJI_SUPPLEMENTAL;
	if (cp >= 0x1F600 && cp <= 0x1FAFF)
		return CAT_EMOJI_EXTENDED;
	return CAT_OTHER;
}
...
static int bundled_width(uint32_t cp) {
	...
	case widechar_nonprint:
		return -1; /* keep -1 for "not printable" */
	...
	case widechar_unassigned:
		return -1; /* treat as "not assigned" */
...
int main(int argc, char **argv) {
	...
	for (int i = 0; i < CAT_COUNT; i++) {
		...
		printf("%-46s %7d %7d %7d %7d%s\n", categories[i].name,
		       categories[i].tested, categories[i].matched,
		       categories[i].mismatched, categories[i].sys_missing,
		       status);
	}
	...
	printf("%-46s %7s %7s %7s\n",
	       "----------------------------------------------", "-------",
	       "-------", "-------");
	printf("%-46s %7d %7d %7d\n", "TOTAL", total_tested, total_matched,
	       total_mismatched);
	printf("\n");
