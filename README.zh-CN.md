...
使用支持 OSC 52 的终端客户端时，`Ctrl-c` 会将选中文本同时复制到板剪切环与系统剪贴板。

## 编辑大文件

`emil` 并非为编辑超大文件而设计。

`emil` 会根据可配置的限制（默认为 1 GB）跟踪内存使用情况。此限制（“预算”）计算所有打开的缓冲区、剪切环以及撤销/重做数据的实际文本内容。命令历史等微小分配不计入其中。

当打开新文件将超过预算时，操作将被拒绝。当删除（kill）或粘贴（yank）操作使总使用量超过预算时，状态栏将显示警告。
...
显示宽度按行缓存，仅在该行编辑时重算。**累积屏幕行缓存将逻辑行映射至屏幕位置，从而在开启自动换行时实现高效滚动。**
...
欢迎提交缺陷修复、可移植性改进、性能优化及代码质量提升的 PR。请勿提议任何新功能。
