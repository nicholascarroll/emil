...
static const char *const msg_no_more_kill_entries   = "剪切环中没有更多条目可粘贴！";
...
static const char *const msg_unsaved_quit           = "有未保存的更改。确定要退出吗？（y 或 n）";
static const char *const msg_buffer_modified_kill   = "缓冲区 %.20s 已修改；仍要关闭吗？（y 或 n）";
...
static const char *const msg_no_more_kill_entries   = "No more kill ring entries to yank!";
...
static const char *const msg_dir_not_supported      = "Directory editing not supported.";
...
static const char *const msg_backward_regex_todo    = "Backward regex search not yet implemented";
...
static const char *const msg_shell_exit_status      = "Shell command exited with status %d";
static const char *const msg_pipe_unavailable       = "Pipe command not available on this platform";
...
static const char *const msg_trailing_removed       = "%d trailing characters removed";
...
static const char *const msg_unsaved_quit           = "There are unsaved changes. Really quit? (y or n)";
static const char *const msg_buffer_modified_kill   = "Buffer %.20s modified; kill anyway? (y or n)";
