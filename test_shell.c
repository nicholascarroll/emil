...
static char *run_shell(const char *shell_cmd, const char *input) {
	...
	int rc = subprocess_create(
		command_line, subprocess_option_inherit_environment, &proc);
	if (rc != 0) {
		fprintf(stderr,
			"    run_shell: subprocess_create failed for '%s'\n",
			shell_cmd);
		return NULL;
	}
	...
	if (subprocess_join(&proc, &sub_ret) != 0) {
		fprintf(stderr,
			"    run_shell: subprocess_join failed for '%s'\n",
			shell_cmd);
...
static char *run_command(const char *const argv[], int *exit_code) {
	...
	if (rc != 0) {
		fprintf(stderr,
			"    run_command: subprocess_create failed for '%s'"
			" (rc=%d, errno=%d: %s)\n",
	...
	if (subprocess_join(&proc, &sub_ret) != 0) {
		fprintf(stderr,
			"    run_command: subprocess_join failed for '%s'\n",
			argv[0]);
		subprocess_destroy(&proc);
		return NULL;
	}
...
static struct buffer *output_to_buffer(const char *output) {
	...
	for (size_t i = 0; i < len; i++) {
		if (output[i] == '\n') {
			insertRow(buf, buf->numrows, (char *)&output[rowStart],
				  rowLen);
			rowStart = i + 1;
			rowLen = 0;
		} else {
...
static char *write_temp_file(const char *content) {
	...
	if (fd == -1) {
		fprintf(stderr,
			"    write_temp_file: mkstemp failed for '%s': %s\n",
			path, strerror(errno));
		free(path);
		return NULL;
	}
	...
	while (total < len) {
		...
		if (n < 0) {
			if (errno == EINTR)
				continue;
			fprintf(stderr,
				"    write_temp_file: write failed: %s\n",
				strerror(errno));
			close(fd);
			unlink(path);
			free(path);
